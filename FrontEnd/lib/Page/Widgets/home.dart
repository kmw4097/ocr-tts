          ...
          child: Column(
            ...
            children: [
              ...
              uiset.isclikedpdf
                  ...
                  : const SizedBox(), // 선택한 파일이 PDF가 아닌 경우 빈 컨테이너 표시
              Column(
                children: [
                  Divider(
                      height: 30, thickness: 2, color: uiset.backgroundcolor),
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                    children: [
                      InkWell(
  ...
  return section == 0
      ? SizedBox(
          child: GetBuilder<FromBackend>(builder: (_) {
            return StreamBuilder(
              stream: fb.Fetchtextorvoice('text'),
              builder: (context, snapshot) {
                if (snapshot.hasData) {
                  return Text(
      ...
      : SizedBox(
          child: GetBuilder<FromBackend>(builder: (_) {
            return StreamBuilder(
              stream: fb.Fetchtextorvoice('voice'),
              builder: (context, snapshot) {
                if (snapshot.hasData) {
                  return Text(
                    snapshot.data as String,
